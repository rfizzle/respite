...
2. **Rate** — let `n` = non-spectator, non-idle players currently in the Overworld and `k` = those with `isSleeping() == true` (in bed, at any stage of falling asleep). The target rate is:
   ...
   With the default `maxTimeLapseRate = 20` and `n = 4`: 1 sleeper → 5×, 2 → 10×, 3 → 15×, 4 → 20×. A solo player gets the full 20×. `k = 0` (or `n = 0`) → rate 1, time-lapse inactive.
...
A full night (12,000 ticks ≈ 10 real minutes) passes in ≈30 seconds at the default 20×. A furnace smelts the same items over that night as vanilla would across a real 10-minute night — because the ticks are real. The default is deliberately calm and TPS-stable; a server that wants a faster (or slower) night raises or lowers `maxTimeLapseRate`, bounded by what the tick budget below can afford.
...
3. **Deep Sleep** — how dark the moon is decides how much each conversion heals. The heal multiplier is a linear ramp in lunar darkness, `1 + (newMoonHealMultiplier − 1) × (4 − |moonPhase − 4|) / 4`, applied to the same 1.0 saturation. The new moon (phase 4) is the peak at the full `newMoonHealMultiplier` (default 2.0 — a full heart per point of saturation, the headline 10-saturation full heal); the full moon (phase 0) is the floor at ×1.0 (plain, no bonus); the phases between step down evenly — crescents ×1.75, quarters ×1.5, gibbous nights ×1.25 at the default peak. The multiplier is read per conversion step from the level's current moon phase.
...
7. **Wake feedback** — if a night's sleep restored ≥ 6 health (3 hearts), the player gets `✦ You wake refreshed` (`notification.respite.rested`) on the action bar at wake; if any conversion ran under the new moon (phase 4, Deep Sleep's peak), the line is `✦ You wake deeply rested` (`notification.respite.deep_rested`) instead, same threshold. The deeper-rested line is the new moon's crown alone — the gradient's lesser nights say only refreshed.
...
- **Blood Moon disjointness** — Deep Sleep peaks at the new moon (phase 4) and falls to exactly no bonus at the full moon (phase 0); Tribulation's Blood Moon keys to the full moon — where Deep Sleep gives nothing — so the two never reinforce one night (see Compatibility).
...
4. **Updates** — the block re-checks its level on a self-rescheduled 20-tick block tick and emits neighbor updates only when the level changes (at most once per 1,600 world ticks in real time; during a 20× time-lapse a change can land every ~4 real seconds — still trivially cheap). Placement sets the correct level immediately.
...
6. **Half-strength Restful Saturation** — while an armed bedroll sleeper (§2) converts, each step heals `bedrollRestfulMultiplier ×` the bed amount (default 0.5) for the same 1.0 saturation. The multiplier stacks with the Deep Sleep gradient, so a bedroll on a new moon heals ×1.0 — the same as a full bed on the full moon — and a real bed always beats it on any night (a full bed on the new moon heals ×2.0). A real bed remains the best night's sleep.
...
- Restful-saturation accounting: 20-interval night totals, stop conditions (saturation floor, full health), penalty-exemption arithmetic, Deep Sleep gradient arithmetic — the per-phase multiplier ramp (new-moon peak, full-moon floor, the crescent/quarter/gibbous steps) at the default and range-extreme peaks.
...
- Restful Saturation: simulated sleeping player with full hunger heals 1.0 HP per 600 world ticks and spends saturation 1:1; vanilla regen suspended while asleep; on a phase-4 night (`/time set` to a new-moon night) each conversion heals 2.0 HP for the same 1.0 saturation, and on a mid-gradient night (a quarter moon) each heals the ramped 1.5 HP while the wake line stays plain refreshed.
