...
| `commands/` | Slash commands (`/glyph`, `/sfx`, `/implement`, `/align`, `/assess`), **vendored from concord** — refreshed by the same `make sync`, never edit here. |
| `agents/` | Reviewer/recon agent definitions, **vendored from concord** — refreshed by the same `make sync`, never edit here. |
...
Domain-specific review emphasis (e.g. the time-lapse engine's per-tick budget
and the vanilla-parity guarantee when feature toggles are off) lives in
